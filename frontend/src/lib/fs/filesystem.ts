...
export async function getBrowserFSLibrariesMounts(archiveNames: string[]) {
    ...
    const fetchData = async (url: string) => {
        console.log(`Fetching library archive: ${url}`);
        const res = await fetch(url);
        if (!res.ok) {
            console.error(`Failed to fetch ${url}: ${res.status} ${res.statusText}`);
            throw new Error(`Failed to fetch ${url}: ${res.status}`);
        }
        console.log(`Fetched ${url}, size: ${res.headers.get('content-length')} type: ${res.headers.get('content-type')}`);
        // Check for common SPA fallback issue
        const contentType = res.headers.get('content-type');
        if (contentType && contentType.includes('text/html')) {
            console.error(`Fetched HTML instead of ZIP for ${url}. This usually means 404 fallback.`);
            throw new Error(`Fetched HTML instead of ZIP for ${url}`);
        }
...
export async function symlinkLibraries(archiveNames: string[], fs: any, prefix = '/libraries', cwd = '/tmp') {
...
export async function createEditorFS({ prefix, allowPersistence }: { prefix: string, allowPersistence: boolean }): Promise<any> {
