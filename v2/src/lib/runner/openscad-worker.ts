...
import { deployedArchiveNames } from "../fs/zip-archives";
import { fetchSource } from "../utils";

import * as BrowserFS from 'browserfs';

// Expose BrowserFS globally for Emscripten filesystem interaction
// @ts-ignore
...
self.addEventListener('message', async (e: MessageEvent<OpenSCADInvocation>) => {
    ...
    try {
        ...
        if (mountArchives) {
            ...
            const BFS = new (self as any).BrowserFS.EmscriptenFS(
                instance.FS,
                instance.PATH ?? {
